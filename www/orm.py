...
#创建一个全局的连接池，每个HTTP请求都可以从连接池中直接获取数据库连接,连接池由全局变量__pool存储，缺省情况下将编码设置为utf8，自动提交事务
...
class Model(dict, metaclass=ModelMetaclass):
    ...
    def getValue(self, key):
        return getattr(self, key, None)     #默认内置函数实现的
